...
public class EnemyControl : MonoBehaviour {
    ...
    void GetStateControl(EnemyState curSate) { 
        
        if(curSate == EnemyState.RUN || curSate == EnemyState.PAUSE) {

            if(curSate != EnemyState.ATTACK) {

                Vector3 targetPosition = new Vector3(playerTarget.position.x, transform.position.y, playerTarget.position.z); // transform in the y axis because the height doesn't matter
        ...
        } else if(curSate == EnemyState.ATTACK) {
        ...
        } else if(curSate == EnemyState.GOBACK) {
            ...
            if(Vector3.Distance(targetPosition, initialPosition) <= 3.5f) {
                enemy_LastState = curSate;
                curSate = EnemyState.WALK;
            }
        } else if(curSate == EnemyState.WALK) {
